...
public class ReportPanel extends JFrame {
	...
	public void showReport(int time, int steps, int duplicateSteps, double coverage, int sumOfWater, int sumOfCoffe,
		...
		if (isWriteToExcel) {
			System.out.println("water: " + MainGui.getNumOfWater());
			System.out.println("coffe: " + MainGui.getNumOfCoffe());
			System.out.println("soda: " + MainGui.getNumOfSoda());
			DataExcel dataExcel = new DataExcel(MainGui.getSizeOfMap(), time, steps, duplicateSteps, coverage,
					sumOfWater, sumOfCoffe, sumOfSoda);
			MainGui.setPointrage((double) (im * 10 + happy * 7 + normal * 5 + bad * 3)
					/ ((im + happy + normal + bad + angry) * 10) * 100);
			DataExcel dataExcel2 = new DataExcel(MainGui.getPoint(), im, happy, normal, bad, angry, MainGui.getPointrage(),
